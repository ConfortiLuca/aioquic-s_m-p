        ...
        for datagram in datagrams:
           ...
           #DEBUG2*********************
            if self._is_client and self._handshake_complete and self._loss._pto_count > 0:   #Max n probe attempt usually 4 or little more due to congestion control new reno and idle timeout
                ...
                """Another solution more complex in terms of computing
                while True:
                    index_addr = random.randint(0, self._max_addr_pool_server_migration)
                    if index_addr < len(self._pool_addr_server_migration):
                        if not self._pool_addr_server_migration[index_addr].probed:
                            print("Probe a possible new server address:")
                            print(self._pool_addr_server_migration[index_addr].addr)
                            ret.append((datagram, self._pool_addr_server_migration[index_addr].addr))
                            self._pool_addr_server_migration[index_addr].probed = True
                            if self._cont_old_server_attempt == 0:
                                self._reset_probing()
                            break
                    else:
                        if self._cont_old_server_attempt == 1:
                            print("Probe old server address")
